...
def experiment(variant):
    # NOTE: This is calling the dict envs for the variant's specified 'env'.,
    #       A string name for a given taks,
    #       This returns a lambda which is passed the variant's dr (T/F flag
    #       to use domain randomization).
    #       Immediately, we wrap this in a class that edits the `observe` method
    #       to apply the transformation method to the RLBench observation. Either running
    #       the observation through the MLR network or scaling it to [-1, 1]. 
    expl_env = envs[variant['env']](variant['dr'])
    expl_env = TransformObservationWrapper(expl_env, variant['main_t_fn'])

    # NOTE: Oh man. This one is a lot NO CLUE WHAT THIS IS YET.
    #       It is one of the algorithm inputs. 
    #       [ ] Figure this one out???
    ...
    # NOTE: Here is where they define their CNN network structure
    #       These get passed after the Q-Function kwargs and after the
    #       policy kwargs, in their respective initializations.
    # obs_dim = expl_env.observation_space.low.size
    action_dim = expl_env.action_space.low.size
    if variant['mlf']:
        if variant['alt'] == 'both':
            # NOTE: Alt implies using the two shoulder cams (I think)
            conv_args = {
                    "input_width": 16,
                    "input_height": 16,
                    "input_channels": 16,
        ...
        if variant['alt'] == 'both':
            ...
            # NOTE: The 'no Mid-Level Features' settings
            conv_args = {
                    "input_width": 64,
                    "input_height": 64,
                    "input_channels": 3,
    ...
    # NOTE: Jokes on us. They have the 'qf_kwargs' and 'policy_kwarg' elements
    #       in their dictionary here... but they are empty! No additional key word
    #       arguments are defined here or in ANY other part of the code.
    #       
    #       Joke's on me! The state.py file might implement everything here but much nicer
    #       If so what a wase of time...
    #
    #       But for what is is worth that file defines:
    #           qf_kwargs=dict(
    ...
    # NOTE: We wrap the exploration strategy with the policy and use this as
    #       an input into the Path collectors. This wrapper is unchanged.
    ...
    # NOTE: The role of these PathCollectors is unclear to me. There is some
    #       relationship to MDPs but I'm not sure what exactly.
    ...
    # NOTE: This class is where the TD3 algorithm is implemented. I.e.,
    #       the logic for when to update the networks.
    trainer = TD3Trainer(
        policy=policy,
        qf1=qf1,
        qf2=qf2,
    ...
    # NOTE: No clue why this is unused... Can it be as simple an un-commenting?
    trainer = HERTrainer(trainer)

    # NOTE: This is packaging to run the algorithm and collect data.
    #       Experience replay is added here, it is not part of the actual Td3
    #       Algorithm, but something more, hence why it is added here and not above.
...
if __name__ == "__main__":
    ...
    # NOTE: This is how they set up multiprocessing
    envs = {
        'reach': reach,
    ...
    # # ------------------------------------------------------------------------ # #
    # # ------------------------------------------------------------------------ # #
    # # NOTE: This is a very important piece of information                      # #
    # #       [ ] Copy this to a table or something where it ies easier to read. # #
    # #       [ ] Still unclear how the mid-level representations are fed in...  # #
    # # ------------------------------------------------------------------------ # #
    # # ------------------------------------------------------------------------ # #
    variant = dict(
        ...
        env=args.env,
        dr = args.dr,
        feature_task=args.feature_task,         # NOTE: VAlue in dict is unused
        noise=args.noise,
        mlf = args.mlf,
        trial = args.trial,
    ...
    # ----------------------------------------------------------------------------------------- #
    # NOTE: This logic selects the functions                                                    #
    #           - t_fn(obs: UNKNOWN TYPE)                                                       #
    #           - main_t_fn(obs: UNKNOWN TYPE)                                                  #
    #       I'm sure they do something. They are passed as arguments into the following classes #
    #       and references in the experiment function.                                          #
    #           - expl_env = TransformObservationWrapper                                        #
    #           - replay_buffer = imgObsDictRelabelingBuffer                                    #
    # ----------------------------------------------------------------------------------------- #
    # single view cases
    ...
    if args.mlf or args.readout:
        ...
        # NOTE: This is probably the transformation definition we want to use
