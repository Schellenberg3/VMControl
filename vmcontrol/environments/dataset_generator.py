...
# NOTE This is what told me they were using Gym 0.10.10
reach                = lambda dr, ss, alt: gym.make('reach_target_easy-visiondepthmask%s-v0' % dr_flag(dr),
                                                     altview=alt,
...
push_rotate          = lambda dr, ss, alt: gym.make('slide_block_to_target-visiondepthmask%s-v0' % dr_flag(dr),
                                                     altview=alt,
                                                     fixed_grip=0,
                                                     rand_config=rand_config,
                                                     img_size=img_size,
                                                     force_randomly_place=False,
                                                     force_change_position=False)
push_free            = lambda dr, ss, alt: gym.make('slide_block_to_target-visiondepthmask%s-v0' % dr_flag(dr),
                                                     altview=alt,
                                                     fixed_grip=0,
                                                     rand_config=rand_config,
                                                     img_size=img_size,
                                                     force_randomly_place=False,
                                                     force_change_position=False)
pick_and_lift        = lambda dr, ss, alt: gym.make('pick_and_lift-visiondepthmask%s-v0' % dr_flag(dr),
                                                     altview=alt,
                                                     rand_config=rand_config,
                                                     sparse=True,
                                                     not_special_p=0.5,
                                                     ground_p = 0.5,
                                                     special_is_grip=True,
                                                     img_size=img_size,
                                                     force_randomly_place=False,
                                                     force_change_position=False)
pick_and_lift_nograb = lambda dr, ss, alt: gym.make('pick_and_lift-visiondepthmask%s-v0' % dr_flag(dr),
                                                     altview=alt,
                                                     rand_config=rand_config,
                                                     sparse=True,
                                                     not_special_p=0.5,
                                                     ground_p = 0.5,
                                                     special_is_grip=False,
                                                     img_size=img_size,
                                                     force_randomly_place=False,
                                                     force_change_position=False)
procedural           = lambda dr, ss, alt: gym.make('empty_container-visiondepthmask%s-v0'% dr_flag(dr),
                                                     altview=alt,
                                                     img_size=img_size,
                                                     not_special_p=0.5,
                                                     ground_p = 0.5,
...
def env_thread(args, thread_num, partition=True, use_ppo2=False):
    ...
    """
    print('[%d] Made it to thread start' % thread_num)
    env_kwargs = {
    ...
    # NOTE: The following error of CoppeliaSim crashing was largely resolved by importing RLKit
    #       and PyTorch libraries _after_ the environment was created. Unsure why this was a fix
    #       but it does seem to work.
    # 
    # print('[%d] [DEBUG] Made it to wrapped_env line' % thread_num)
    # NOTE: This is the line with our error inside of it!
    #       This references the lambda function, which eventually reaches RLBenchEnv.__init__
    env_kwargs["wrapped_env"] = envs[args.env](args.dr, args.special_start, args.alt)

    # print('[%d] [DEBUG] Made it to RLBenchDSEnv line' % thread_num)
    ...
    # print('[%d] [DEBUG] Made it to episode loop start' % thread_num)
...
def main():
    ...
    if args.num_cpu > args.num_episode:
        ...
        print("num_cpu cannot be greater than num_episode, defaulting to {} cpus.".format(args.num_cpu))
