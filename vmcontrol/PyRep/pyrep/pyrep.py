...
class PyRep(object):
    ...
    def launch(self, scene_file="", headless=False, responsive_ui=False,
        ...
        # 
        # print("[DEBUG] Started PyRep launch")
        ...
        # print("[DEBUG] Set up the thread")
        
        self._ui_thread.start()

        # print("[DEBUG] Started the thread")

        # NOTE/TODO:    This is where the program is stopping.
        #               The `sim` identifier is a module with many PyRep
        #               function calls in it, this one included.
        #               It accesses the sim_cffi library which is the 
        #               C Foreign Function Interface. So this is how we
        #               actually call the C scripts.
        while not sim.simExtCanInitSimThread():
            time.sleep(0.1)

        # NOTE: The sleep timer seems to be for CoppeliaSim to load
        #       all if its required libraries. This may be seen by running
        #       any of the RLBench scripts.
        #       Somehow, this fails for out needs.
        # print("[DEBUG] Passed CanInit")

        sim.simExtSimThreadInit()
        time.sleep(0.2)  # Stops CoppeliaSim crashing if restarted too quickly.

        # print("[DEBUG] Made it to blocking check")
